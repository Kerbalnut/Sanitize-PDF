...
ECHO DEBUGGING: _GOT_ADMIN = '%_GOT_ADMIN%'
...
ECHO DEBUGGING: _CHOICES_BEFORE_ELEVATION = '%_CHOICES_BEFORE_ELEVATION%'
::- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
IF /I NOT "%_CHOCO_CMD_RESULT%"=="SUCCESS" ( 
	...
	ECHO DEBUGGING: Continue on with rest of script from here...
	PAUSE
...
ECHO DEBUGGING: End of :gswin64c_install function.
...
::- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
:: 4b. Check if the gswin64c.exe exists in Program Files directory
ECHO DEBUGGING: Looking for gswin64c.exe in Program Files "%ProgramFiles%"
	...
	IF EXIST "%_GSWIN64C_EXE%" (
		...
		IF /I "%_GOT_ADMIN%"=="YES" (
			IF EXIST "%ChocolateyInstall%\tools\shimgen.exe" (
				...
				ECHO DEBUGGING: Adding gswin64c to PATH via SETX . . .
				REM SETX PATH "%PATH%;%_FOLDER%" && SET "_COMMAND_EXIT=SUCCESS" || SET "_COMMAND_EXIT=FAILURE"
				ECHO DEBUGGING: Adding gswin64c to gswin64c via SETX . . .
				REM SETX gswin64c "%_GSWIN64C_EXE%" && SET "_COMMAND_EXIT=SUCCESS" || SET "_COMMAND_EXIT=FAILURE"
				ECHO DEBUGGING: %%_COMMAND_EXIT%% = %_COMMAND_EXIT%
				IF /I NOT "%_COMMAND_EXIT%"=="SUCCESS" (
					ECHO Failed to add "%_FOLDER%" to PATH with SETX.
					ECHO:
					REM PAUSE
					REM GOTO END
				)
				IF /I NOT "%_COMMAND_EXIT%"=="SUCCESS" (
					ECHO DEBUGGING: Adding gswin64c to PATH via PowerShell . . .
					&"%SystemRoot%\System32\WindowsPowerShell\v1.0\powershell.exe" -NoProfile -ExecutionPolicy Bypass -Command "[Environment]::SetEnvironmentVariable('path',"%_FOLDER%;$([Environment]::GetEnvironmentVariable('path','Machine'))",'Machine');"
					ECHO DEBUGGING: %%PATH%% = %PATH%
				)
			)
		) ELSE (
			ECHO DEBUGGING: Adding gswin64c to PATH . . .
			REM SETX PATH "%PATH%;%_FOLDER%" && SET "_COMMAND_EXIT=SUCCESS" || SET "_COMMAND_EXIT=FAILURE"
			ECHO DEBUGGING: Adding gswin64c to gswin64c via SETX . . .
			REM SETX gswin64c "%_GSWIN64C_EXE%" && SET "_COMMAND_EXIT=SUCCESS" || SET "_COMMAND_EXIT=FAILURE"
			ECHO DEBUGGING: %%_COMMAND_EXIT%% = %_COMMAND_EXIT%
			IF /I NOT "%_COMMAND_EXIT%"=="SUCCESS" (
				ECHO Failed to add "%_FOLDER%" to PATH.
				ECHO:
				PAUSE
				REM GOTO END
			)
...
::- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
:: 4c. Check if the gswin64c.exe exists in Program Files (x86) directory
ECHO DEBUGGING: Looking for gswin64c.exe in Program Files "%ProgramFiles(x86)%"
IF NOT EXIST "%_GSWIN64C_EXE%" SET "_GSWIN64C_EXE="
SET "_FOLDER="
ECHO DEBUGGING: "%ProgramFiles(x86)%\gs\gs*.**\bin\gswin64c.exe"
...
IF /I "%_GSWIN64C_INSTALLED%"=="NO" (
	...
	IF EXIST "%_BOXSTARTER_INSTALLER%" (
		...
		ECHO DEBUGGING: Boxstarter script found: 
		ECHO DEBUGGING: "%_BOXSTARTER_PATH%"
	) ELSE (
		ECHO DEBUGGING: Our dependency still needs to be installed, but a Boxstarter script cannot be found.
	)
)
::- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
:: 6. Cast errors if our External Function is still not found.
ECHO DEBUGGING: Call errors and installers if script is still not found . . .
IF /I "%_GSWIN64C_INSTALLED%"=="NO" (
	ECHO:
...
::- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
ECHO DEBUGGING: End of gswin64c.exe External Function.
:GSWIN64C_SKIP
:-------------------------------------------------------------------------------
...
ECHO DEBUGGING: %%_INPUT_PDF%% = %_INPUT_PDF%
ECHO DEBUGGING: %%_INPUT_PDF_NAME%% = %_INPUT_PDF_NAME%
ECHO DEBUGGING: %%_INPUT_PDF_PATH%% = %_INPUT_PDF_PATH%
ECHO DEBUGGING: %%_INPUT_PDF_SIZE%% = %_INPUT_PDF_SIZE% B
ECHO DEBUGGING: %%_INPUT_PDF_SIZE_KB%% = %_INPUT_PDF_SIZE_KB% KB
...
ECHO DEBUGGING: gswin64c -sDEVICE=pdfwrite
ECHO DEBUGGING: gswin64c -sOutputFile="%%_OUTPUT_PDF%%" "%%_INPUT_PDF%%"
ECHO DEBUGGING: gswin64c -sOutputFile="%_OUTPUT_PDF%" "%_INPUT_PDF%"
...
ECHO DEBUGGING: gswin64c -sDEVICE=ps2write
ECHO DEBUGGING: gswin64c -sOutputFile="%%_PLACEHOLDER_PS%%" "%%_INPUT_PDF%%"
ECHO DEBUGGING: gswin64c -sOutputFile="%_PLACEHOLDER_PS%" "%_INPUT_PDF%"
...
ECHO DEBUGGING: gswin64c -sDEVICE=pdfwrite
ECHO DEBUGGING: gswin64c -sOutputFile="%%_OUTPUT_PDF_PS%%" "%%_PLACEHOLDER_PS%%"
ECHO DEBUGGING: gswin64c -sOutputFile="%_OUTPUT_PDF_PS%" "%_PLACEHOLDER_PS%"
...
ECHO DEBUGGING: gswin64c -sDEVICE=pdfwrite
ECHO DEBUGGING: gswin64c -dDownsampleColorImages -dColorImageDownsampleType=/Bicubic -dColorImageResolution=%_DPI% -dDownsampleGrayImages -dGrayImageDownsampleType=/Bicubic -dGrayImageResolution=%_DPI% -dDownsampleMonoImages -dMonoImageDownsampleType=/Bicubic -dMonoImageResolution=%_DPI%
ECHO DEBUGGING: gswin64c -sOutputFile="%%_OUTPUT_PDF_IMAGES%%" "%%_INPUT_PDF%%"
ECHO DEBUGGING: gswin64c -sOutputFile="%_OUTPUT_PDF_IMAGES%" "%_INPUT_PDF%"
...
ECHO DEBUGGING: %%_INPUT_SIZE%% = %_INPUT_SIZE%
FOR %%G IN ("%_OUTPUT_PDF%") DO SET "_OUTPUT_SIZE=%%~zG"
ECHO DEBUGGING: %%_OUTPUT_SIZE%% = %_OUTPUT_SIZE%
::https://ss64.com/nt/set.html
SET /A _SIZE_DIFF=%_INPUT_SIZE%-%_OUTPUT_SIZE%
ECHO DEBUGGING: %%_SIZE_DIFF%% = %_SIZE_DIFF%
